    ...
    async function (request, user_email, user_password, done) {
      ...
      User.findOne({ email: user_email })
        ...
        .then(async user => {
          ...
          console.log(
            "!user || !bcrypt.compareSync(user_password, user.password) : ",
            !user || !bcrypt.compareSync(user_password, user.password)
          );
          console.log("!user : ", !user);
          console.log(
            "!bcrypt.compareSync(user_password, user.password)",
            !user || !bcrypt.compareSync(user_password, user.password)
          );
          ...
          if (!user || (await bcrypt.compare(user_password, user.password))) {
            ...
            if (user.source != "Email") {
              return done(null, false, {
                message:
                  "You have previously signed up with a different sign in method",
              });
            }
            request.user = user;
...
passport.use(
  new MagicLinkStrategy(
    ...
    function send(user, token) {
      ...
      const link =
        "https://internconnect-fdoz.onrender.com/user/register/email/verify?token=" +
        token;
      const msg = {
        to: user.email,
        ...
        text:
          "Hello! Click the link below to finish signing in to Intern Connect.\r\n\r\n" +
          link,
    ...
    function verify(user, token) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        newUser = User.One({ email: user.email })
          .exec()
          .then(newUser => {
            console.log("User: ", user);
  ...
  new GoogleStrategy(
    {
      ...
      callbackURL:
        "https://internconnect-fdoz.onrender.com/auth/google/register",
